...
已编写独立的 Supabase 库存迁移、受身份验证保护的领取接口、用户四步流程和管理员库存面板。**本地验证完成不代表生产数据库已启用**；未配置时用户不会取得代码，界面明确提示。原有 `admin_messages` 人工申请记录保留在后台历史区，新流程不再写入该收件箱。

### 启用步骤

1. 在 Supabase SQL Editor 执行 `supabase/migrations/20260830_play_testing_codes.sql`。脚本可重复执行，不重置分配；当前管理员需要 `profiles.role = admin`。
2. 管理后台 → 测试与兑换码 → 导入未使用的代码 CSV。每个 App 独立导入，填写 Google 活动的真实开始/结束时间，确认代码尚未兑换、未人工发出。默认暂停，不会立刻分发。
3. 已经手动发送、兑换或泄露的代码不得导入为可用库存；可通过停用名单排除。重复导入不会重置、回收或跨 App 移动代码。
4. 在 Play Console 核实该活动 Active 与有效期，再勾选后台确认、启用批次。网站不能通过本地库存确认 Google 的实际兑换状态。
5. 仅使用明确指定的测试账号和测试码验收真实端到端流程，不随意消耗正式库存。

### 用户流程与权限边界

网站验证邮箱必须与 Google Play 使用邮箱一致（支持非 Gmail 的 Google 账户）。同一个账户加入 Google 群并完成每 App 的 opt-in 后，在网站勾选声明并领取；重复操作返回原码，不另消耗库存。群成员资格和 Google opt-in **由用户完成并声明，网站尚未自动验证或代加群**。

每用户/每 App、每邮箱散列/每 App 和每代码唯一。Gmail 的点号、加号别名与 googlemail.com 归一，避免简单别名重复领取；这不是对自然人身份的保证。取码事务使用用户行锁、代码行 `FOR UPDATE SKIP LOCKED` 和数据库唯一约束。删账户不回收曾发出的代码。停用、活动暂停和过期时不继续显示代码为可兑换。

代码保存在不对客户端开放的 `play_private` 表中，以便将原码返还给拥有者；**本版没有额外的应用层字段加密**。RLS、schema权限及限权函数保护库存。普通登录用户不能查询其他用户或全库代码；管理员默认看到统计与领取邮箱，不显示全库原码。邮箱去重值保存 SHA-256 摘要，记录只向管理员展示。对代码与日志仍按敏感信息管理。

接口无缓存；身份来自验证 JWT，不接受客户端提交其他 user id。无新 service-role 环境变量要求。真实代码不得进入 GitHub、前端 bundle、public、测试文件或日志。

### 已做验证与限制

`npm test` 包含 CSV 导入与错误隐私验证。`scripts/verify-play-code-inventory.mjs` 在临时 PGlite 数据库、合成代码上验证权限、重复领取、跨用户不同码、邮箱别名、暂停、停用、到期、重复导入及删号不回收。PGlite 单连接，**不是多连接压力测试**。尚未用生产真实代码做兑换验收，也未确认用户现有代码是否已使用、活动有效期或 Active 状态。

通用法语图解指南：`public/guides/installation-gratuite-google-play.pdf`。全部截图已遮去邮箱、卡片信息、设备型号和使用日期。
...
## 历史方案：未来若考虑收费交付（未实现、未启用）

**以下是早期付费交付设计，不是当前免费测试的行为或数据结构。**
