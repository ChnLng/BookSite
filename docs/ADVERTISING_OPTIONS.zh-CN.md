...
2026-08-30 站主的线上截图出现交友引流、VPN 连接警告及清理存储提示素材，与图书和教育品牌不符。成人广告开关此前已关闭，但并未阻止这些素材。

- `adsterraEnabled = false` 是本站总开关。所有 Ads 组件均不挂载广告加载器，历史同意记录也不能绕过暂停；公开广告块一并隐藏，避免留下空白大块。
- 单元 `30994241`、原广告位组件、脚本和隔离逻辑保留，未删除平台账号，也未代为改变平台后台设置。已打开旧页面需刷新；刷新前可点击“Retirer mon accord”立即撤下当前页面广告。
- 恢复前须让平台确认并执行类别排除、核查素材和落地页，并更换同意记录版本要求重新同意；不能仅凭关闭 Adult ads 或改为 Banner/1:1 就恢复。
- 更适合本站的替代路线是站主审核过的图书、语言学习用品联盟推荐，或审核过的直接赞助素材。联盟推荐按有效成交结算，不等于按展示获得收入；未取得专属获批链接前不冒充已接入。
...
- 首页、目录及图书／工具详情页曾接入站主提供的 Adsterra Native Banner，现已暂停。单元 ID `30994241`，网站 ID `6017866`，平台截图曾显示 Active、Books 类别、成人广告关闭。平台状态与本站总开关独立。
- 保留的加载器在启用后还要求访客同意、正式域名和广告位实际可见；同意记录有效期为 180 天。暂停状态优先于这些条件，因此当前任何页面都不挂载广告 iframe。
- 保留的隔离 iframe 不授予 `allow-same-origin`，防止访问网站登录存储和表单。这项隔离不能筛选第三方广告素材，也不能保证内容符合品牌。
...
| [Adsterra](https://adsterra.com/blog/set-up-publishers-dashboard/) | 官方不设最低流量要求；支持 Banner、Native Banner 等，禁止机器和激励流量。需站主注册、提交网站、取得专属广告代码。 | 本站实测素材不适合品牌，现已暂停，不推荐在未经确认的情况下恢复。成人开关和网站 Books 类别不足以控制所有素材。不得启用 Popunder、强制跳转或通知诱导。 |
...
布局是平台侧配置，选择 1:1 后必须点击 SAVE，但不会因此限定广告主题。当前保持暂停，不为验收制造广告展示或点击。若未来恢复时广告服务拒绝隔离 iframe，应向平台确认受支持的隔离方式，不直接取消登录数据保护。
