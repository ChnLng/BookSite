...
type ProductReport = { kind: string; ref: string; name: string; active: boolean; sales: number; downloads: number; revenue: number; lastPurchasedAt: string | null; lastDownloadedAt: string | null; lastBuyer: string | null; lastAmount: number | null; currency: string };
    ...
    <div className="section-block">
      ...
      <div className="admin-report-table-wrap">
        <table className="admin-report-table">
          <thead><tr><th>类别 / 商品</th><th>销量</th><th>下载总数</th><th>最后购买用户</th><th>购买时间</th><th>该用户最后下载</th><th>支付金额</th></tr></thead>
          <tbody>
...
function FragmentRows({ section, onDelete }: { section: SectionReport; onDelete: (product: ProductReport) => Promise<void> }) {
  return <>
    <tr className="admin-report-section-row"><th>{section.title}</th><th>{section.sales}</th><th>{section.downloads}</th><th colSpan={3}>类别总收入</th><th>{section.revenue.toFixed(2)} EUR</th></tr>
    {section.products.map((product) => <tr key={`${product.kind}-${product.ref}`}>
      <td><strong>{product.name}</strong>{!product.active ? <><span className="tiny"> · 已从商品页删除</span><button className="report-delete-button" type="button" onClick={() => void onDelete(product)}>删除统计历史</button></> : null}</td><td>{product.sales}</td><td>{product.downloads}</td><td>{product.lastBuyer || "—"}</td><td>{dateTime(product.lastPurchasedAt)}</td><td>{dateTime(product.lastDownloadedAt)}</td><td>{product.lastAmount == null ? "—" : `${product.lastAmount.toFixed(2)} ${product.currency}`}</td>
    </tr>)}
  </>;
}
