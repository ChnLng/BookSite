...
const itchUrls: Record<string, string> = {
  "com.visdar.calendrier": "https://visdar.itch.io/calendrier",
  "com.visdar.heures": "https://visdar.itch.io/heures",
  "com.visdar.manuscrits": "https://visdar.itch.io/reconnaissance",
  "com.visdar.couleurs": "https://visdar.itch.io/couleur",
  "com.visdar.famille": "https://visdar.itch.io/parente",
};
const googlePlayTestHint = "Pour consulter cette fiche Google Play pendant la phase de test, rejoignez d’abord le groupe Visd AR avec le compte Google utilisé dans Play Store.";
...
export function AndroidCatalogueBook({ config, kind }: { config: CatalogueConfig; kind: CatalogueKind }) {
  ...
  pages.push({ id: "couverture", title: "Couverture", content: <div className="collection-cover">
    <div className="cover-copy"><span className="collection-eyebrow">Applications Android · {business ? "Établissements & professionnels" : "La collection"}</span><h1>{config.title}</h1><p className="collection-lead">{config.introduction}</p>{business ? <aside className="collection-business-test-note"><strong>Accès aux fiches Google Play</strong><p>Les applications sont actuellement en test. Avant d’ouvrir leurs fiches Google Play, rejoignez le groupe Visd AR avec le compte Google utilisé dans votre Play Store.</p>{external(playTestingGroupUrl, "Rejoindre le groupe de test", true)}</aside> : null}{jump(1, <>Ouvrir le catalogue <ArrowRight size={19} /></>, "collection-button primary")}<span className="cover-languages">汉字 <i /> Pinyin <i /> Français</span></div>
  ...
  apps.forEach((app, index) => {
    const itchUrl = itchUrls[app.packageName];
    const googlePlayUrl = `https://play.google.com/store/apps/details?id=${app.packageName}`;
    pages.push({ id: app.packageName.split(".").pop()!, title: app.title, theme: themes[index % themes.length], content: <div className="collection-app-spread"><AppArt app={app}/><div className="collection-app-copy"><span className="collection-eyebrow">La collection · 0{index + 1}</span><h2>{app.title}</h2><p className="collection-tagline">{app.subtitle}</p><p>{app.description}</p><ul className="collection-features">{app.features.map(feature => <li key={feature}><Check size={16}/>{feature}</li>)}</ul><p className="collection-audience">{app.audience}</p>{business && itchUrl ? <div className="collection-app-actions">{external(itchUrl, "Ouvrir sur itch.io")}{external(googlePlayUrl, "Découvrir sur Google Play", true, googlePlayTestHint)}</div> : external(googlePlayUrl, "Découvrir sur Google Play", true)}<small className="collection-fine">Disponibilité selon le compte, l’appareil et le pays. Visuel d’ambiance : l’interface de l’application peut différer.</small></div></div> });
  });
  if (business) {
