...
function CreationAbout() {
  ...
  return <div className="collection-about"><div className="collection-about-heading"><span className="collection-eyebrow">Une intention pédagogique · 教学初衷</span><h2>À propos<br/><em>de cette création.</em></h2><p>让课堂之外的学习，也有清晰、可靠而自在的延续。</p></div><div className="collection-about-sections">{sections.map((section, index) => <div className="collection-about-pair" key={index}><section lang="fr"><span>Français</span><p>{section.french}</p></section><section lang="zh-CN"><span>中文</span><p>{section.chinese}</p></section></div>)}</div><p className="collection-about-signature">Visd AR · 汉字 · Pinyin · Français</p></div>;
}
