...
function CreationAbout() {
  const sections = [
    ...
    {
      french: "Depuis 2018, la créatrice enseigne le chinois dans des grandes écoles d’ingénieurs, des écoles de commerce, des universités et des associations. Au contact des étudiants, elle a constaté combien ils avaient besoin d’outils mieux ajustés à leur manière d’apprendre. Après un travail attentif de sélection, d’organisation et d’amélioration continue, les premières ressources en ligne de 2025 prennent désormais la forme d’applications Android, publiées en 2026.",
      chinese: "制作人自 2018 年起，先后在工程师大学校、商学院、大学和协会教授汉语。与学生长期相处，让她深知学习者需要更贴合自身理解方式的工具。经过认真筛选、系统整理与不断优化，2025 年的网页版资源终于在 2026 年发展为正式上线的 Android 手机版应用。",
    },
  ];
  return <div className="collection-about"><div className="collection-about-heading"><span className="collection-eyebrow">Une intention pédagogique · 教学初衷</span><h2>À propos<br/><em>de cette création.</em></h2><p>让课堂之外的学习，也有清晰、可靠而自在的延续。</p></div><div className="collection-about-sections">{sections.map((section, index) => <div className="collection-about-pair" key={index}><section lang="fr"><p>{section.french}</p></section><section lang="zh-CN"><p>{section.chinese}</p></section></div>)}</div></div>;
}

export function AndroidCatalogueBook({ config, kind, presentation = "standard", mobileDescriptions = {} }: { config: CatalogueConfig; kind: CatalogueKind; presentation?: "standard" | "mobile"; mobileDescriptions?: Record<string, string> }) {
