...
export async function POST(request: Request) {
  ...
  try {
    ...
    const response = await fetch(`${paypalBaseUrl()}/v2/checkout/orders/${orderId}`, { headers: { Authorization: `Bearer ${token}` }, cache: "no-store" });
    const order = await response.json() as any;
    if (!response.ok || order.status !== "COMPLETED") throw new Error("Le paiement PayPal n'est pas confirme.");
    ...
    const { error } = await supabase.from("donations").insert({ user_id: user?.id || null, user_name: payerName || user?.email?.split("@")[0] || null, user_email: order.payer?.email_address || user?.email || null, amount, currency: capture?.amount?.currency_code || "EUR", note: String(payload?.note || "Soutien libre").slice(0, 160), payment_status: "paid", paid_at: capture?.create_time || new Date().toISOString(), paypal_order_id: orderId, paypal_capture_id: capture?.id || payload?.captureId || null });
